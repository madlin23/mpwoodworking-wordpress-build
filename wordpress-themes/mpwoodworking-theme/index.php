    ...
    <?php if ( have_posts() ) : ?>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
            <?php while ( have_posts() ) : the_post(); ?>
                <article <?php post_class('border border-[#2a2a28] hover:border-[#a3e635]/30 transition-colors p-6 bg-[#11110f] relative'); ?>>
                    <!-- Grüner Akzentpunkt oben rechts -->
                    <div class="absolute top-4 right-4 w-1.5 h-1.5 rounded-full bg-[#a3e635] shadow-[0_0_4px_#a3e635]"></div>
                    ...
                    <h2 class="font-serif text-3xl font-black uppercase mb-3 text-[#f8f8f7]">
                        <a href="<?php the_permalink(); ?>" class="hover:text-[#a3e635] transition-colors"><?php the_title(); ?></a>
                    </h2>
                    ...
                    <a href="<?php the_permalink(); ?>" class="text-xs tracking-wider uppercase font-bold text-[#f8f8f7] hover:text-[#a3e635] flex items-center space-x-1 font-sans group">
