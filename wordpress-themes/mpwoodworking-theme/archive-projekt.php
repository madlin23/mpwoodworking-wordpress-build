...
<!-- Projekte Header -->
<section class="bg-[#11110f] border-b border-[#2a2a28] py-16">
        ...
        <?php if ( have_posts() ) : ?>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
                <?php while ( have_posts() ) : the_post(); ?>
                    <div class="group flex flex-col space-y-4 border border-[#2a2a28] p-4 bg-[#11110f] hover:border-[#d40924] transition-colors">
                        <div class="aspect-[16/10] w-full overflow-hidden bg-[#1a1a19] border border-[#2a2a28] relative">
                            ...
                            <?php endif; ?>
                            <span class="absolute top-4 left-4 bg-[#010101] text-[#f8f8f7] text-[10px] tracking-widest uppercase px-3 py-1 border border-[#2a2a28] font-bold">
                                Projekt <?php echo get_the_date('Y'); ?>
                            </span>
                        </div>
                        <div class="flex flex-col space-y-2 pt-2">
                            <h3 class="font-serif text-2xl font-black uppercase text-[#f8f8f7] group-hover:text-[#d40924] transition-colors">
                                <?php the_title(); ?>
                            </h3>
                            <div class="flex flex-col space-y-1 text-xs text-[#a8a8a3] font-sans">
                                <?php if ( function_exists('get_field') && get_field('holzart_detail') ) : ?>
                                    <span class="font-bold uppercase tracking-wider text-[10px]">Holzart: <span class="font-light text-[#f8f8f7]"><?php the_field('holzart_detail'); ?></span></span>
                                <?php endif; ?>
                            </div>
                        </div>
                        ...
                        <div class="pt-2">
                            <a href="<?php the_permalink(); ?>">
                                <span class="text-xs tracking-wider uppercase font-bold text-[#f8f8f7] group-hover:text-[#d40924] flex items-center space-x-1 font-sans">
