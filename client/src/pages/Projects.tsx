        ...
        <div className="container">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
            {projects.map((project) => (
              <div key={project.id} className="group flex flex-col space-y-4 border border-[#2a2a28] p-4 bg-[#11110f] hover:border-[#008b1d]/40 transition-colors relative">
                {/* Feine grüne Akzentlinie links neben dem Textblock bei Hover */}
                <div className="absolute left-0 top-4 bottom-4 w-[2px] bg-[#008b1d] opacity-0 group-hover:opacity-100 transition-opacity" />
                ...
                <div className="pt-2">
                  <Link href={`/projekt/${project.slug}`}>
                    <span className="text-xs tracking-wider uppercase font-sans font-bold text-[#f8f8f7] hover:text-[#008b1d] cursor-pointer flex items-center space-x-1 transition-colors">
